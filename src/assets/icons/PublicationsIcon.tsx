// SVGIcon.js
const PublicationsIcon = ({ width = 28, height = 28, color = '#757575' }) => (
  <svg
    width={width}
    height={height}
    viewBox='0 0 28 28'
    fill='none'
    xmlns='http://www.w3.org/2000/svg'
  >
    <path
      fillRule='evenodd'
      clipRule='evenodd'
      d='M17.1928 3.22217H9.43101C7.02884 3.22217 4.95801 5.16933 4.95801 7.57267V20.0712C4.95801 22.6098 6.89351 24.634 9.43101 24.634H18.7515C21.1548 24.634 23.102 22.4757 23.102 20.0712V9.3775L17.1928 3.22217Z'
      stroke={color}
      strokeWidth='1.5'
      strokeLinecap='round'
      strokeLinejoin='round'
    />
    <path
      d='M16.8865 3.2085V6.60233C16.8865 8.259 18.227 9.603 19.8825 9.6065C21.419 9.61 22.9905 9.61116 23.0966 9.60416'
      stroke={color}
      strokeWidth='1.5'
      strokeLinecap='round'
      strokeLinejoin='round'
    />
    <path
      d='M16.6647 18.1507H10.3682'
      stroke={color}
      strokeWidth='1.5'
      strokeLinecap='round'
      strokeLinejoin='round'
    />
    <path
      d='M14.283 12.3731H10.3677'
      stroke={color}
      strokeWidth='1.5'
      strokeLinecap='round'
      strokeLinejoin='round'
    />
  </svg>
)

export default PublicationsIcon
